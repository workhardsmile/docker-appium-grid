...
def get_push_queue
  ...
  #hash_results = JSON.parse(result.body) rescue (return $global_status=true)
  ...
  $logger.info("complete script_task:#{hash_results}")
  $global_status = true
end
  ...
  threads<<Thread.new do
    loop do
      ...
      next
        ...
        when "rspec"
          exec_cmd = "ruby"
          start_cmd = "#{exec_cmd} #{File.join(local_path,'rtest-framework/testing','run.rb')} -e #{script_task.env} -p #{script_task.browser} -s #{File.join(testing_path,script_task.script_name)} -r #{script_task.round_id} -o #{script_task.file_name} -j '#{script_task.to_hash.to_json}' -i #{$testplus_config['ip_addr']}"
        when "ruby"
          exec_cmd = "ruby"
          start_cmd = "#{exec_cmd} #{File.join(local_path,'rtest-framework/testing','run.rb')} -e #{script_task.env} -p #{script_task.browser} -s #{File.join(testing_path,script_task.script_name)} -r #{script_task.round_id} -o #{script_task.file_name} -j '#{script_task.to_hash.to_json}' -i #{$testplus_config['ip_addr']}"
        when "python"
          exec_cmd = "python"
          #branch_name = "dev"
          start_cmd = "#{exec_cmd} #{File.join(local_path,'pytest-framework/interface','testmain.py')} -e #{script_task.env} -p #{script_task.browser} -s #{File.join(testing_path,script_task.script_name)} -r #{script_task.round_id} -o #{script_task.file_name} -j '#{script_task.to_hash.to_json}' -i #{$testplus_config['ip_addr']}"
        when "pyunit"
          exec_cmd = "python"
          start_cmd = "#{exec_cmd} #{File.join(local_path,'pytest-framework/interface','testmain.py')} -e #{script_task.env} -p #{script_task.browser} -s #{File.join(testing_path,script_task.script_name)} -r #{script_task.round_id} -o #{script_task.file_name} -j '#{script_task.to_hash.to_json}' -i #{$testplus_config['ip_addr']}"
        when "maven"||"mvn"||"java"
          exec_cmd = "mvn"
          start_cmd = "bash run.sh -c #{exec_cmd} -e #{script_task.env} -p #{script_task.browser} -s #{File.join(testing_path,script_task.script_name)} -r #{script_task.round_id} -o #{script_task.file_name} -j '#{script_task.to_hash.to_json}' -i #{$testplus_config['ip_addr']} -h #{local_path}"
        else
        next
        end
        ...
        begin
          ...
          when 'git'
            unless File.exist?(local_path)
              if exec_cmd == 'ruby'
                extra_cmd = '&&bundle install' 
              elsif exec_cmd == 'python'
                extra_cmd = '&&pip install -r requirements.txt' 
              end
